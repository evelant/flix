...
class PortableBigDecimalLlvmNativeRuntimeSuite extends AnyFunSuite {
  ...
  test("portable-bigdecimal-llvm-native-runtime") {
    assume(hasZig, "zig not found on PATH (skipping LLVM-native BigDecimal slice runtime test)")

    val driverFile = Files.createTempFile("flix-portable-bigdecimal-llvm-native-driver-", ".flix")
    val outDir = Files.createTempDirectory("flix-llvm-native-portable-bigdecimal-runtime-")
    try {
      ...
      if (exit != 0) {
        fail(s"LLVM-native BigDecimal slice driver failed with exit $exit:\n$output")
      }
    } finally {
  ...
  private def mkDriverSource(): String = {
    ...
    PortableConformance.mkDriverSource(tests, banner = "portable-bigdecimal-llvm-native-runtime")
  }
