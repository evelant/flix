...
/**
  * End-to-end runtime smoke tests for the portable BigInt slice on the LLVM-wasm backend.
  *
  * Compiles a dedicated Flix fixture set to a wasm component and executes it using the Node runner
  * (`tools/wasm-runner-js/run-flix.mjs`).
  */
class PortableBigIntLlvmWasmRuntimeSuite extends AnyFunSuite {
  ...
  private val testsDir = Paths.get("main/test/flix/portable_bigint")

  private val wasmRunnerScript: Path =
    Paths.get("tools/wasm-runner-js/run-flix.mjs").toAbsolutePath.normalize()

  test("portable-bigint-llvm-wasm-runtime") {
    assume(hasZig, "zig not found on PATH (skipping LLVM-wasm BigInt slice runtime test)")
    assume(hasWasmTools, "wasm-tools not found on PATH (skipping LLVM-wasm BigInt slice runtime test)")
    assume(hasJco, "jco not found on PATH (skipping LLVM-wasm BigInt slice runtime test)")
    assume(hasNode, "node not found on PATH (skipping LLVM-wasm BigInt slice runtime test)")

    val driverFile = Files.createTempFile("flix-portable-bigint-llvm-wasm-driver-", ".flix")
    val outDir = Files.createTempDirectory("flix-llvm-wasm-portable-bigint-runtime-")
    val sandboxDir = Files.createTempDirectory("flix-llvm-wasm-portable-bigint-sandbox-")
    try {
      ...
      if (exit != 0) {
        fail(s"LLVM-wasm BigInt slice driver failed with exit $exit:\n$output")
      }
    } finally {
  ...
  private def mkDriverSource(): String = {
    ...
    val root = optRoot.getOrElse {
      throw new IllegalStateException("Expected a TypedAst root for portable BigInt slice.")
    }

    implicit val iflix: Flix = flix
    val tests = PortableConformance.collectPortableTests(root)
    PortableConformance.mkDriverSource(tests, banner = "portable-bigint-llvm-wasm-runtime")
  }
