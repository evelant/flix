...
class PortableTextEncodingLlvmWasmRuntimeSuite extends AnyFunSuite {
  ...
  private val testsDir = Paths.get("main/test/flix/portable_text_encoding")

  private val wasmRunnerScript: Path =
    Paths.get("tools/wasm-runner-js/run-flix.mjs").toAbsolutePath.normalize()

  test("portable-text-encoding-llvm-wasm-runtime") {
    assume(hasZig, "zig not found on PATH (skipping LLVM-wasm text-encoding slice runtime test)")
    assume(hasWasmTools, "wasm-tools not found on PATH (skipping LLVM-wasm text-encoding slice runtime test)")
    assume(hasJco, "jco not found on PATH (skipping LLVM-wasm text-encoding slice runtime test)")
    assume(hasNode, "node not found on PATH (skipping LLVM-wasm text-encoding slice runtime test)")

    val driverFile = Files.createTempFile("flix-portable-text-encoding-llvm-wasm-driver-", ".flix")
    val outDir = Files.createTempDirectory("flix-llvm-wasm-portable-text-encoding-runtime-")
    val sandboxDir = Files.createTempDirectory("flix-llvm-wasm-portable-text-encoding-sandbox-")
    try {
      ...
      if (exit != 0) {
        fail(s"LLVM-wasm text-encoding slice driver failed with exit $exit:\n$output")
      }
    } finally {
  ...
  private def mkDriverSource(): String = {
    ...
    val root = optRoot.getOrElse {
      throw new IllegalStateException("Expected a TypedAst root for portable text-encoding slice.")
    }

    implicit val iflix: Flix = flix
    val tests = PortableConformance.collectPortableTests(root)
    PortableConformance.mkDriverSource(tests, banner = "portable-text-encoding-llvm-wasm-runtime")
  }
