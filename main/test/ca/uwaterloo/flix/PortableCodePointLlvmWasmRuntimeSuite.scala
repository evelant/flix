...
class PortableCodePointLlvmWasmRuntimeSuite extends AnyFunSuite {
  ...
  test("portable-codepoint-llvm-wasm-runtime") {
    assume(hasZig, "zig not found on PATH (skipping LLVM-wasm CodePoint slice runtime test)")
    assume(hasWasmTools, "wasm-tools not found on PATH (skipping LLVM-wasm CodePoint slice runtime test)")
    assume(hasJco, "jco not found on PATH (skipping LLVM-wasm CodePoint slice runtime test)")
    assume(hasNode, "node not found on PATH (skipping LLVM-wasm CodePoint slice runtime test)")

    val driverFile = Files.createTempFile("flix-portable-codepoint-llvm-wasm-driver-", ".flix")
    val outDir = Files.createTempDirectory("flix-llvm-wasm-portable-codepoint-runtime-")
    val sandboxDir = Files.createTempDirectory("flix-llvm-wasm-portable-codepoint-sandbox-")
    try {
      ...
      if (exit != 0) {
        fail(s"LLVM-wasm CodePoint slice driver failed with exit $exit:\n$output")
      }
    } finally {
  ...
  private def mkDriverSource(): String = {
    ...
    PortableConformance.mkDriverSource(tests, banner = "portable-codepoint-llvm-wasm-runtime")
  }
