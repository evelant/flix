...
/**
  * End-to-end runtime smoke tests for the portable BigInt slice on the LLVM-native backend.
  *
  * Compiles+links+executes a small driver that calls the `@Test` functions explicitly.
  */
class PortableBigIntLlvmNativeRuntimeSuite extends AnyFunSuite {
  ...
  test("portable-bigint-llvm-native-runtime") {
    assume(hasZig, "zig not found on PATH (skipping LLVM-native BigInt slice runtime test)")

    val driverFile = Files.createTempFile("flix-portable-bigint-llvm-native-driver-", ".flix")
    val outDir = Files.createTempDirectory("flix-llvm-native-portable-bigint-runtime-")
    try {
      ...
      if (exit != 0) {
        fail(s"LLVM-native BigInt slice driver failed with exit $exit:\n$output")
      }
    } finally {
  ...
  private def mkDriverSource(): String = {
    ...
    PortableConformance.mkDriverSource(tests, banner = "portable-bigint-llvm-native-runtime")
  }
