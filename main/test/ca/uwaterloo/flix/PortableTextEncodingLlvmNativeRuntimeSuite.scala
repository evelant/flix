...
class PortableTextEncodingLlvmNativeRuntimeSuite extends AnyFunSuite {
  ...
  test("portable-text-encoding-llvm-native-runtime") {
    assume(hasZig, "zig not found on PATH (skipping LLVM-native text-encoding slice runtime test)")

    val driverFile = Files.createTempFile("flix-portable-text-encoding-llvm-native-driver-", ".flix")
    val outDir = Files.createTempDirectory("flix-llvm-native-portable-text-encoding-runtime-")
    try {
      ...
      if (exit != 0) {
        fail(s"LLVM-native text-encoding slice driver failed with exit $exit:\n$output")
      }
    } finally {
  ...
  private def mkDriverSource(): String = {
    ...
    PortableConformance.mkDriverSource(tests, banner = "portable-text-encoding-llvm-native-runtime")
  }
